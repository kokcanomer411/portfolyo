...
const response=await fetch('./assets/omer/head-model.json?v=360');if(!response.ok)throw new Error('Model unavailable');const data=await response.json();
...
const pivot=new THREE.Group();scene.add(pivot);const head=new THREE.Group();head.position.z=.65;pivot.add(head);
